...
def demo_leads(city: str, country: str, niche: str, limit: int = 8) -> List[ShopLead]:
    ...
    for name, notes, has_web, variety, ads in expanded[: max(1, limit)]:
        ...
        lead = ShopLead(
            ...
            website=(f"https://{abs(hash(name+city))%10**8}.example-shop.test" if has_web else None),
            has_website=has_web,
            ...
            independence_signals="family run, multi-brand/variety, Instagram product posts",
            pain_points=notes + " | Instagram product showcase / social-first",
            instagram=f"https://instagram.com/shop_{abs(hash(name + city)) % 10**9}",
            facebook=f"https://facebook.com/shop_{abs(hash(name + city + 'fb')) % 10**9}",
            source_url="demo://sample",
            notes=notes + " | locality social discovery",
